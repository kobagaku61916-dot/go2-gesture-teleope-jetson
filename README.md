...
**ステータス: Phase 5〜6 完了（2026-07-08）— Jetson 単体でのジェスチャーテレオペ実機成功。**
低速 → 最高速（0.6/0.8 = クランプ上限）の段階検証を完走し、**最高速を既定化**。
実装済みアクション: テレオペ（前進/後退/旋回・**対面ミラー**）/ **手振り → Greet(お手)** / **交互腕伸ばし → Dance1** / **人追従**（BlazePose 版実機成功 2026-07-09）。

**branch `feature/yolo-follow`（2026-07-13）**: 認識を **YOLO11n-pose TensorRT(GPU, 30fps)** に置換 +
**見失い探索**（消えた方向へ旋回・4s タイムアウト）を実装。dry-run 合格・**実機テストは次回**。
初めて動かす構成では `--low-speed` で検証してから（[runbook.md](runbook.md)）。
...
**全 Phase 完了（2026-07-08）。** 実測記録は [TODO.md](TODO.md) と各 config のコメント参照。
...
| 右手を高く上げる（静止） | 前進 |
| 左手を高く上げる（静止） | 後退 |
| 手を横に伸ばす | **手を出した側**（あなたから見た向き）へ旋回（対面ミラー） |
| 両手を下げる / 視野から出る / 低信頼検出 | **停止**（0 指令） |
| **右手を肩より上で左右に 2 秒振る** | **Greet**（Hello お手, api 1016） |
| **右腕⇄左腕を交互に横へ伸び縮み（5 秒）** | **Dance1**（api 1022） |
